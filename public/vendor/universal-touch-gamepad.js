...
(function() {
    'use strict';
    // Local Demo changes: racing profile and synchronous lifecycle reset hook.
    ...
    window.demoTouchState = () => gamepadState;
    window.demoResetTouch = () => disconnectGamepad();
    ...
    function disconnectGamepad() {
        ...
        for (const reset of controlResetHandlers) reset();
        if (wasConnected) {
    ...
    const profiles = {
        racing: {
            name: "赛车手柄",
            joysticks: [{id:"steering",axes:[0,1],style:{left:"24px",bottom:"36px",size:"112px"}}],
            analogTriggers: [
                {id:"brake",buttonIndex:6,label:"LT 刹车 ↑",direction:"up",style:{right:"104px",bottom:"36px",width:"68px",height:"150px"}},
                {id:"throttle",buttonIndex:7,label:"RT 油门 ↑",direction:"up",style:{right:"22px",bottom:"36px",width:"68px",height:"150px"}}
            ],
            buttons: [
                {id:"demoA",index:0,label:"A",style:{left:"calc(50% - 160px)",bottom:"4px",width:"44px",height:"40px",borderRadius:"8px"}},
                {id:"demoB",index:1,label:"B",style:{left:"calc(50% - 108px)",bottom:"4px",width:"44px",height:"40px",borderRadius:"8px"}},
                {id:"demoX",index:2,label:"X",style:{left:"calc(50% - 56px)",bottom:"4px",width:"44px",height:"40px",borderRadius:"8px"}},
                {id:"demoY",index:3,label:"Y",style:{left:"calc(50% - 4px)",bottom:"4px",width:"44px",height:"40px",borderRadius:"8px"}},
                {id:"demoBack",index:8,label:"BACK",style:{left:"calc(50% + 48px)",bottom:"4px",width:"54px",height:"40px",fontSize:"12px"}},
                {id:"demoStart",index:9,label:"MENU",style:{left:"calc(50% + 110px)",bottom:"4px",width:"54px",height:"40px",fontSize:"12px"}}
            ],
            clusters: [{id:"demoButtons",style:{left:"calc(50% - 164px)",bottom:"0px",width:"332px",height:"48px"},buttonIds:["demoA","demoB","demoX","demoY","demoBack","demoStart"]}]
        },
    ...
    function renderControlElements(profile, parentEl, scale = 1.0, isPreview = false) {
        ...
        if (profile.joysticks) {
            profile.joysticks.forEach(joyConfig => {
                ...
                if (!isPreview) {
                    ...
                    controlResetHandlers.push(() => {
                        activeTouchId = null;
                        base.classList.remove('pressed');
                        const size = base.getBoundingClientRect().width;
                        handle.style.left = `${size * (1 - handleRelSizeFactor) / 2}px`;
                        handle.style.top = `${size * (1 - handleRelSizeFactor) / 2}px`;
                        Object.assign(base.dataset, {touchStartTime:'0',movedSignificant:'false'});
                    });
        ...
        if (profile.analogTriggers && !isPreview) {
            profile.analogTriggers.forEach(triggerConfig => {
                ...
                let activeId = null;
                controlResetHandlers.push(() => { activeId = null; });
        ...
        if (profile.buttons) {
            profile.buttons.forEach(btnConfig => {
                ...
                if (btnConfig.id.startsWith('demo')) button.classList.add('demo-extra-key');
    ...
    function updateAnalogTriggerVisuals(buttonIndex, currentClientY, isActive) {
        ...
        if (isActive && triggerTrack.activeTouchId !== null && triggerHeight > 0) {
            ...
            value = Math.max(0, Math.min(1, triggerTrack.config.direction === 'up' ? 1 - relativeY / triggerHeight : relativeY / triggerHeight));
        }
